...
# Author: Jennifer Holborn <jholborn@student.ethz.ch>
...
def set_pch_vol_cur(instr: Instr, vol: float, cur: float,
        volmin: float, volmax: float, channel: int = 0):
    assert(vol > volmin and vol < volmax)
    if channel == 0:
...
def power_reset_cycle(instrs: dict, psu_configs: dict, ganged : bool = True, 
...
def reconf_siggens(instrs: dict, siggen_configs: dict, 
