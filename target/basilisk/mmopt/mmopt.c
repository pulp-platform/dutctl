...
static inline void set_csricache(int val) {
    asm volatile("csrw 0x7c0, %0" :: "r"(val):"memory");
}

static inline void set_csrdcache(int val) {
    asm volatile("csrw 0x7c1, %0" :: "r"(val):"memory");
}
...
int main(void) {
    ...
    uart_init(&__base_uart, reset_freq, 1200);

    // Launch a supply measurement in 1000ms (middle of compute)
    printf("@dutctl:psumeas:core:1000:E36231A_1:0\r\n");
    fence();
    ...
    // Return core results. If `chkint` does not match, it's a logical test failure.
    printf("@dutctl:dutmeas:cycles:%lu\r\n", cycles);
    printf("@dutctl:dutmeas:instret:%lu\r\n", instret);
    printf("@dutctl:dutmeas:chkint:%lu\r\n", chkint);
    fence();
